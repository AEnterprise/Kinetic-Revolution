...
public abstract class GuiBase extends GuiContainer {
    ...
    protected void drawGuiContainerForegroundLayer(int x, int y) {
        if (drawPlayerInv)
            drawString(StatCollector.translateToLocal("container.inventory"), 8, tileGuiYSize + 6, textColor);
        String name = hasCustomName() && getCustomName() != null ? getCustomName() : StatCollector.translateToLocal(String.format("gui.%s.name", getInventoryName()));
